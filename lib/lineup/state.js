 ...
 * @param {{ foot, target, brk, finish }} actual   — observed result
 */
export function recordShot(planned, actual) {
  const ball      = State.activeBall();
  const ballStart = LaneRead.ballStartFromFoot(actual.foot);
  const line      = new Line(actual.foot, ballStart, actual.target, actual.brk, actual.finish);
 ...
 * @param {{ foot, target, brk, finish }} actual
 */
